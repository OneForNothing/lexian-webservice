    ...
    <body style="background: rgb(254,238,189);">
        ...
        <table border="1" width="100%" cellspacing="0" background="black">
            ...
            <c:forEach items="${requestScope.pageBean.beanList}" var="order">
                <tr align="center" bgcolor="rgb(78,78,78)" bordercolor="rgb(78,78,78)" style="color: white;">
                    <td>${order.o_id}</td>
                    ...
                    <td><a id="a_detail"
                           href="<c:url value='/seller/SellerOrderServlet?method=viewDetail&o_id=${order.o_id}&state=${o_state}'/>">查看详情</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
        ...
        <%--<td colspan="6">--%>
        <%--订单编号：${order.o_id}--%>
        <%--下单时间：${order.o_orderTime}--%>
        <%--订单总额：<font color="red"><b>${order.o_totalPrice}</b></font>--%>
        ...
        <%--<c:when test="${order.o_state eq 2}">已完成</c:when>--%>
        <%--</c:choose>--%>
        <%--&nbsp;&nbsp;<a href="<c:url value="/seller/SellerOrderServlet?method=viewDetail&o_id=${order.o_id}&state=${o_state}"/>">查看详情</a>--%>
        <%--</td>--%>
        <%--</tr>--%>
        <%--</c:forEach>--%>
    </body>
