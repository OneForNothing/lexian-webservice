...
public class SellerDao {
    ...
    public void add(Seller form) {
        try {
            String sql = "insert into seller (s_id, s_name, s_pwd, s_email, s_code) values(?,?,?,?,?)";
            qr.update(sql, form.getS_id(), form.getS_name(), form.getS_pwd(),
                    form.getS_email(), form.getS_code());
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public List<Seller> findByStoreName(String keyword) {
        try {
            String sql = "select * from seller where s_storeName like '%" + keyword + "%'";
    ...
    public void likeSeller(String u_id, String s_id) {
        try {
            String sql = "insert into likeseller values(?,?)";
